...
<head>
  ...
  .styled-button:active {
    ...
    .delete-btn, .logout-btn, .print-btn, .download-btn {
    ...
    .delete-btn:hover, .logout-btn:hover, .print-btn:hover, .download-btn:hover {
  ...
  <script>
    ...
    function downloadOrders() {
      ...
      orders.forEach((order, index) => {
        ...
        const drinkPrice = drinkPrices[order.drink] || 0;
        const totalAmount = (foodPrice + drinkPrice) * order.quantity;
        doc.text(`Order ${index + 1}: ${JSON.stringify(order)} | Amount: ${totalAmount}`, 10, 20 + (index * 10));
      });
      doc.save('orders.pdf');
    }
    ...
    function filterOrders(radio) {
      ...
      rows.forEach(row => {
        ...
        if (status === 'all' || (status === 'delivered' && delivered) || (status === 'pending' && !delivered)) {
          row.style.display = '';
        } else {
    ...
    function printReceipt() {
      ...
      doc.text("Digital Lunchbox Receipt", 105, 30, { align: "center" });
      ...
      doc.setFontSize(10);
      doc.setFont("helvetica", "bold");
      doc.text("All Rights Reserved © 2024, Digital Lunchbox ™", 105, yPosition + 10, { align: "center" });
